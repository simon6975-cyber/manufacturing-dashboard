...
const SortableRow: React.FC<{
  ...
  const style: React.CSSProperties = {
    ...
    backgroundColor: isDragging ? 'rgba(59, 130, 246, 0.08)' : undefined,
  ...
  return (
    ...
    >
      {children((listeners || {}) as React.HTMLAttributes<HTMLButtonElement>)}
    </tr>
  );
};
...
const DragHandle: React.FC<{ handleProps: React.HTMLAttributes<HTMLButtonElement> }> = ({ handleProps }) => (
  <button
    {...handleProps}
    className="opacity-30 group-hover:opacity-100 transition-opacity cursor-grab active:cursor-grabbing text-gray-500 hover:text-gray-200 p-1"
    title="드래그하여 순서 변경"
...
const HistoryPanel: React.FC<{
  ...
  loading: boolean;
}> = ({ open, onClose, history, loading }) => {
  if (!open) return null;

  const actionLabel: Record<CodeHistoryEntry['action'], string> = {
    save: '저장',
    restore: '기본값 복원',
    init: '초기화',
  };
  const actionColor: Record<CodeHistoryEntry['action'], string> = {
    save: 'bg-blue-500/20 text-blue-300 border-blue-500/30',
    restore: 'bg-amber-500/20 text-amber-300 border-amber-500/30',
    init: 'bg-gray-500/20 text-gray-300 border-gray-500/30',
  };
  ...
  return (
    <>
      ...
      <div className="fixed top-0 right-0 h-full w-full max-w-md bg-gray-950 border-l border-gray-800 z-50 overflow-y-auto shadow-2xl">
        {/* 헤더 */}
        <div className="sticky top-0 bg-gray-950 p-4 border-b border-gray-800 flex items-center justify-between z-10">
        ...
        <div className="p-4 space-y-2">
          {loading ? (
            <div className="text-center py-12">
              ...
              <p className="text-xs text-gray-500 mt-2">이력 로딩 중...</p>
            </div>
          ) : history.length === 0 ? (
            <div className="text-center py-12">
              <Clock className="w-10 h-10 mx-auto text-gray-700 mb-3" />
              <p className="text-sm text-gray-500">아직 변경 이력이 없습니다.</p>
              <p className="text-xs text-gray-600 mt-1">코드를 수정하고 저장하면 여기에 기록됩니다.</p>
            </div>
          ) : (
            history.map((entry) => (
              <div
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  <span
                    className={`text-[10px] font-bold px-2 py-0.5 rounded border tracking-wider ${actionColor[entry.action]}`}
                  >
                    {actionLabel[entry.action]}
                  </span>
                  <span className="text-[11px] text-gray-500" title={formatDate(entry.timestamp)}>
                    {timeAgo(entry.timestamp)}
                  </span>
                </div>
                <p className="text-sm text-gray-200 mb-1.5 leading-relaxed">{entry.summary}</p>
...
export default function CodesPage() {
  ...
  const hasChanges = useMemo(() => {
    return (
      JSON.stringify(stopCodes) !== JSON.stringify(baselineStop) ||
      JSON.stringify(bottleneckCodes) !== JSON.stringify(baselineBn)
    );
  ...
  // ============================================
  useEffect(() => {
    // localStorage 에서 사용자 이름 로드
    if (typeof window !== 'undefined') {
      const savedName = localStorage.getItem('codesPage_userName');
    ...
    (async () => {
      try {
        const [sc, bc] = await Promise.all([loadStopCodes(), loadBottleneckCodes()]);
        const stopData = sc && sc.length > 0 ? sc : defaultStopCodes;
        const bnData = bc && bc.length > 0 ? bc : defaultBottleneckCodes;
        setStopCodes(stopData);
        setBottleneckCodes(bnData);
        setBaselineStop(stopData);
        setBaselineBn(bnData);
      } catch (err) {
        ...
        setBaselineStop(defaultStopCodes);
        setBaselineBn(defaultBottleneckCodes);
      } finally {
        setLoading(false);
      }
    })();
  ...
  // 사용자 이름 변경 시 localStorage 저장
  useEffect(() => {
    if (typeof window !== 'undefined') {
      localStorage.setItem('codesPage_userName', userName);
    }
  }, [userName]);

  // 페이지 이탈 경고 (저장 안 된 변경 사항이 있을 때)
  useEffect(() => {
  ...
  const handleStopDragEnd = (event: DragEndEvent) => {
    ...
    if (over && active.id !== over.id) {
      const oldIndex = stopCodes.findIndex((c) => c.id === active.id);
      const newIndex = stopCodes.findIndex((c) => c.id === over.id);
      if (oldIndex !== -1 && newIndex !== -1) {
        setStopCodes(arrayMove(stopCodes, oldIndex, newIndex));
      }
    }
  };

  const handleBnDragEnd = (event: DragEndEvent) => {
    const { active, over } = event;
    if (over && active.id !== over.id) {
      const oldIndex = bottleneckCodes.findIndex((c) => c.id === active.id);
      const newIndex = bottleneckCodes.findIndex((c) => c.id === over.id);
      if (oldIndex !== -1 && newIndex !== -1) {
        setBottleneckCodes(arrayMove(bottleneckCodes, oldIndex, newIndex));
      }
    }
  };
  ...
  const updateStopCode = <K extends keyof StopCodeV2>(id: string, key: K, value: StopCodeV2[K]) => {
  ...
  const addBottleneckCode = () => {
    const newCode: BottleneckCodeV2 = {
      id: generateId(),
  ...
  const updateBottleneckCode = <K extends keyof BottleneckCodeV2>(id: string, key: K, value: BottleneckCodeV2[K]) => {
  ...
  // 변경 요약 생성
  // ============================================
  const calculateChangeSummary = useCallback((): string => {
    const stopAddedIds = new Set(stopCodes.filter((c) => !baselineStop.find((b) => b.id === c.id)).map((c) => c.id));
    const stopRemovedCnt = baselineStop.filter((c) => !stopCodes.find((b) => b.id === c.id)).length;
    const stopEditedCnt = stopCodes.filter((c) => {
      if (stopAddedIds.has(c.id)) return false;
      const base = baselineStop.find((b) => b.id === c.id);
      return base && JSON.stringify(base) !== JSON.stringify(c);
    }).length;
    const stopReordered =
      stopCodes.length === baselineStop.length &&
      stopAddedIds.size === 0 &&
      stopRemovedCnt === 0 &&
      stopCodes.some((c, i) => baselineStop[i]?.id !== c.id);

    const bnAddedIds = new Set(
      bottleneckCodes.filter((c) => !baselineBn.find((b) => b.id === c.id)).map((c) => c.id)
    );
    const bnRemovedCnt = baselineBn.filter((c) => !bottleneckCodes.find((b) => b.id === c.id)).length;
    const bnEditedCnt = bottleneckCodes.filter((c) => {
      if (bnAddedIds.has(c.id)) return false;
      const base = baselineBn.find((b) => b.id === c.id);
      return base && JSON.stringify(base) !== JSON.stringify(c);
    }).length;
    const bnReordered =
      bottleneckCodes.length === baselineBn.length &&
      bnAddedIds.size === 0 &&
      bnRemovedCnt === 0 &&
      bottleneckCodes.some((c, i) => baselineBn[i]?.id !== c.id);

    const parts: string[] = [];
    if (stopAddedIds.size > 0) parts.push(`정지코드 ${stopAddedIds.size}개 추가`);
    if (stopRemovedCnt > 0) parts.push(`정지코드 ${stopRemovedCnt}개 삭제`);
    if (stopEditedCnt > 0) parts.push(`정지코드 ${stopEditedCnt}개 수정`);
    if (stopReordered) parts.push('정지코드 순서 변경');
    if (bnAddedIds.size > 0) parts.push(`병목코드 ${bnAddedIds.size}개 추가`);
    if (bnRemovedCnt > 0) parts.push(`병목코드 ${bnRemovedCnt}개 삭제`);
    if (bnEditedCnt > 0) parts.push(`병목코드 ${bnEditedCnt}개 수정`);
    if (bnReordered) parts.push('병목코드 순서 변경');
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await Promise.all([
        saveStopCodes(stopCodes),
        saveBottleneckCodes(bottleneckCodes),
        addHistoryEntry({ user: userName, action: 'save', summary }),
      ]);
      setBaselineStop(stopCodes);
      setBaselineBn(bottleneckCodes);
      alert(`✅ 저장 완료!\n\n${summary}`);
    } catch (err) {
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="flex items-start justify-between flex-wrap gap-4">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              {/* 작업자 이름 */}
              <div className="flex items-center gap-1.5 bg-gray-900 border border-gray-800 rounded px-2 py-1.5">
                ...
                <input
                  ...
                  placeholder="작업자"
                  className="bg-transparent text-sm text-gray-200 w-20 outline-none placeholder:text-gray-600"
                />
              </div>
          ...
          {loadError && (
            <div className="bg-red-950/30 border border-red-900/50 rounded-lg p-3 flex items-start gap-2">
              ...
              <div className="text-sm">
                <p className="text-red-300 font-medium">Firebase에서 데이터를 불러올 수 없어 기본값을 표시합니다.</p>
                <p className="text-red-400/80 text-xs mt-1">에러: {loadError}</p>
              </div>
            </div>
          )}
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                ...
                <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleStopDragEnd}>
                  <SortableContext items={stopCodes.map((c) => c.id)} strategy={verticalListSortingStrategy}>
                    <tbody>
                      {stopCodes.map((code) => (
                        <SortableRow key={code.id} id={code.id} disabled={!code.active}>
                          {(handleProps) => (
                            <>
          ...
          <section className="bg-gray-900/60 rounded-lg border border-gray-800">
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                ...
                <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleBnDragEnd}>
                  <SortableContext items={bottleneckCodes.map((c) => c.id)} strategy={verticalListSortingStrategy}>
                    <tbody>
                      {bottleneckCodes.map((code) => (
                        <SortableRow key={code.id} id={code.id} disabled={!code.active}>
                          {(handleProps) => (
                            <>
                              <td className="py-2 px-1 text-center">
          ...
          <div className="text-xs text-gray-600 pb-4">
            💡 좌측 ⋮⋮ 아이콘을 <strong>드래그</strong>해서 순서를 변경할 수 있습니다. 명칭/설명은 클릭해서 바로 수정 가능합니다.
          </div>
        </div>
      </main>
