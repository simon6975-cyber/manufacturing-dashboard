...
export default function TransactionPage() {
  ...
  useEffect(() => {
    fetch('/data/operations.json')
      .then(res => res.json())
      .then(json => { setData(json.records || []); setClients(json.clients || []); setLoading(false); })
  ...
  return (
    <div className="flex flex-col gap-4 p-5 bg-black min-h-full">
      <div className="flex items-start justify-between">
        ...
        {!loading && <span className="text-[10px] text-gray-600 pt-1">총 {data.length.toLocaleString()}건</span>}
      </div>
